    ...
    viewport={{once:true}}>
      ...
      transition={{delay:1, duration:1}}>
        {Array(6).fill("").map((item, index)=>(
            <motion.img className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10' src={index % 2 ? assets.sample_img_2 : assets.sample_img_1} key={index} width={70} alt="" whileHover={{scale:1.05, duration:0.1}} />
        ))}
      </motion.div>
      ...
      initial={{opacity:0}}
      animate={{opacity:1}}
      transition={{delay:1.2, duration:0.8}}>Gererated images from imagify</motion.p>
    </motion.div>
  )
}
